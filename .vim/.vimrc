...
function! SetWindowsOptions()
  ...
  " Note: If I update vim, may need to replace the file into the colors folder
  ...
  " Toggle the shell path between powershell and cmd
  " Note: the ampersand (&) is used to set the variable
  "       directly.  set shell= will not work in this
  "       context
  let g:powershell_path = 'C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe'
  let g:cmd_path = 'C:\Windows\System32\cmd.exe'
  let g:using_powershell = 1

  function! ToggleShell()

    if g:using_powershell
      let &shell=g:powershell_path
      let g:using_powershell = 0
    else
      let &shell=g:cmd_path
      let g:using_powershell = 1
    endif
  ...
  " **********************************
  " * WINDOWS AUTOCOMMANDS
  " **********************************

  " Starts gvim as a maximized window
...
" Opens a new window with notes about commands I want to learn
  function! ShowCommandList()
    ...
    6split ~/Dropbox/.vim/commands.md
  ...
  " Runs ctags at project root and sets tags to the generated file
  function! RunCtags()

    if has("gui_win32")
      execute("!ctags -R C:\Users\abilson\Dropbox\AccountingProgram\src\*")
      set tags="C:\Users\abilson\Dropbox\AccountingProgram\src\tags"
    endif
  ...
  " Sets ctags to an easy-to-use map
  " disabling for now
  " nnoremap <Leader>e :call RunCtags()<CR>
  ...
  if has("gui_win32")
    execute("%w ! cat | pandoc -f markdown -t html | clip")
  endif
...
" }
" }
" # IMPORT PLUGINS WITH VUNDLE {
" ##################################
function! ImportPluginsWithVundle()

  " This has been deprecated in favor of native package loading

  " Required setting (for some reason)
  filetype on
  filetype off

  " Add Vundle to the runtimepath
  let &runtimepath = '~/Dropbox/.vim/bundle/Vundle.vim' . ',' . &runtimepath
  ...
  if has("python")
    " Ultisnips requires python 2.7 or above
    Plugin 'SirVer/ultisnips'
  endif

  Plugin 'vim-syntastic/syntastic'

  " Plugins for coding specific languages
  Plugin 'vim-scripts/Pydiction'
  Plugin 'pangloss/vim-javascript'
  Plugin 'mxw/vim-jsx'
  Plugin 'leshill/vim-json'
  " Enable this later as a linter tool
  " Plugin 'w0rp/ale'

  " Ends Vundle execution?
  call vundle#end()

  " Allows filetype plugins
  filetype plugin on
...
function! ExecuteSettings()
  ...
  " for every instance of vim.  Replaces /bundle path with /plugin
  "execute pathogen#infect('~/Dropbox/.vim/bundle/{}')
